...
class LogToPolars(Converter):
    ...
    def create_dataframe(self, log_filename: str, usecols: Optional[List[str]] = None) -> pl.DataFrame:
        ...
        # 3. Get dataframe.
        self._df =  self._get_dataframe(log_filename=log_filename, all_fields=all_fields, dtypes=type_map, usecols=usecols)
    ...
    def _get_dataframe(self, log_filename: str, all_fields: List[str], dtypes: Dict, usecols: Optional[List[str]] = None) -> pl.DataFrame:
        """Internal Method: Create the initial dataframes by using Pandas read CSV (primary types correct)"""
        return pl.read_csv(log_filename, separator='\t', has_header=False, new_columns=all_fields, columns=usecols, schema_overrides=dtypes, comment_prefix="#", null_values=["-", "NA", ""])
