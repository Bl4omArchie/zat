...
class ZeekLogReader(file_tailer.FileTailer):
    ...
    def __init__(self, filepath, delimiter="\t", tail=False, strict=False):
        ...
        self.type_mapper = {
            "bool": lambda x: True if x == "T" else False,
            "count": int,
            "int": int,
            "double": float,
        ...
        self.dash_mapper = {
            ...
            "port": 0,
            "double": 0.0,
            "time": datetime.datetime.fromtimestamp(86400),
    ...
    def make_dict(self, field_values):
        ...
        """
        data_dict = {}
        for key, value, field_type, converter in zip(
            self.field_names, field_values, self.field_types, self.type_converters
        ):
            try:
